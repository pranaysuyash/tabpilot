...
- Recovery and remediation work for this session is performed on `staging`, not directly on `main`.
- Validation happens on `staging`, then the branch is pushed and reviewed via a PR targeting `main`.
- Only after PR review is complete should `staging` be merged into `main`.
- Temporary backup branches may be created during safety operations, but should be deleted once the branch/PR state is stabilized.
